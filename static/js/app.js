...
function renderTagList(value, filterClass) {
  ...
  if (filterClass) {
    return items
      .map(
        (item) =>
          `<span class="tag" style="cursor:pointer;" onclick="setFilterByClick('${filterClass}', '${escapeHtml(item)}')">${escapeHtml(item)}</span>`
      )
      .join(' ');
  }
...
function renderColumnValue(asset, column) {
  ...
  const commentBadge = meta.comment.trim() ? '<span class="local-comment-indicator" title="Local comment saved">📝</span>' : '';
...
async function fetchServerAssetsPage(pageNum, state) {
  ...
  document.getElementById('resultsInfo').textContent = `${shown} asset${shown > 1 ? 's' : ''} shown out of ${total}`;
}
...
async function startBatchDetails() {
  ...
  try {
    ...
    if (!missingUids.length) {
      alert(targetUids.length > 0 ? 'Selected/Filtered assets already have full details!' : 'All assets already possess full details!');
      return;
    }

    const scopeMsg =
      targetUids.length > 0 ? `${missingUids.length} missing among the ${targetUids.length} queried asset(s)` : `${missingUids.length} assets`;
...
async function clearCache() {
  if (
    !confirm('WARNING: Are you sure you want to clear the entire local cache? All asset data will be deleted and you will need to fetch it again.')
  ) {
    return;
  }
  try {
...
async function applyFiltersLegacy(state) {
  ...
  result.sort((a, b) => {
    ...
    switch (sort) {
      ...
      case 'type_desc':
        return (b.listing_type || '').localeCompare(a.listing_type || '');
      case 'format_asc':
        return (a.asset_formats || '').localeCompare(b.asset_formats || '');
      case 'format_desc':
        return (b.asset_formats || '').localeCompare(a.asset_formats || '');
      case 'date_asc':
  ...
  document.getElementById('resultsInfo').textContent = `${result.length} asset${result.length > 1 ? 's' : ''} shown out of ${total}`;
}
...
function renderTable(assets, options) {
  ...
  if (pageCount > 1 || totalForPagination > 20) {
    ...
    const itemsPerPageOptions = [20, 50, 100]
      .map((val) => `<option value="${val}" ${itemsPerPage === val ? 'selected' : ''}>${val} per page</option>`)
      .join('');
...
async function goToPage(pageNum) {
  const state = collectFiltersState();
  const pageCount =
    isServerModeActive && !requiresLegacyModeFromState(state) ? serverTotals.pageCount : Math.ceil(filteredAssets.length / itemsPerPage);
...
function resolveCustomExportExtension(profileName, pattern) {
  ...
  const looksCsv = name.includes('csv') || (fmt.includes(',') && !fmt.includes('|') && !fmt.includes(']('));
  const looksMarkdown = name.includes('markdown') || fmt.includes('| %') || fmt.includes('- [%') || fmt.includes('](%');
